...
const Datalist = ({ dataList, fetchData, searchValue, pagination, loading, userPermissions, onEditSuccess, onEdit }) => {
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm("Voulez-vous supprimer ce navire ?")) {
      try {
        let url = `${URLS.INCIDENT_API}/ships/${id}`;
  ...
  const handleSaveEdit = async (record) => {
    ...
    setIsSaving(true);
    try {
      const url = `${URLS.INCIDENT_API}/ships/${record.id}`;
