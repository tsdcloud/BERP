...
const Datalist = ({ dataList, fetchData, searchValue, pagination, loading, userPermissions, onEditSuccess, onEdit }) => {
  ...
  const canEditConsumable = () => {
    if (!userPermissions) return true;
    return userPermissions.includes("consumable__can_edit");
  };

  const canDeleteConsumable = () => {
    if (!userPermissions) return true;
    return userPermissions.includes("consumable__can_delete");
  };

  const handleDelete = async (id) => {
    if (!canDeleteConsumable()) {
      toast.error("Vous n'avez pas la permission de supprimer");
      return;
    }
    if (window.confirm("Voulez-vous supprimer ce consommable ?")) {
      try {
        const url = `${URLS.INCIDENT_API}/consumables/${id}`;
  ...
  const handleSaveEdit = async (record) => {
    ...
    setIsSaving(true);
    try {
      const url = `${URLS.INCIDENT_API}/consumables/${record.id}`;
  ...
  const columns = [
    ...
    },
    {
      ...
      fixed: 'right',
      render: (value, record) => {
        // const canEdit = canEditConsumable();
        const canEdit = true;
        const canDelete = canDeleteConsumable();
                  ...
                  <DropdownMenuItem 
                    ...
                    onClick={() => {
                      if (onEdit) {
                        ...
                        setEditFormData({ name: record.name });
                      }
                    }}
                  >
    ...
    <div className="w-full">
      <div className="py-2 px-4 w-full max-h-[500px]">
        <Table 
          ...
          locale={{ emptyText: "Aucun consommable trouvé" }}
        />
      </div>
    </div>
  );
