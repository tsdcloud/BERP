...
// import { ArchiveBoxIcon, Bars3BottomLeftIcon, ChevronLeftIcon, ChevronRightIcon, Cog6ToothIcon, ExclamationTriangleIcon, RectangleStackIcon, TruckIcon, WrenchScrewdriverIcon, CpuChipIcon, LifebuoyIcon } from '@heroicons/react/24/outline';
...
//         {
//             name:"Navires",
//             isActive:pathname.includes("ships") ? true : false,
...
import { Drawer } from 'antd';
import { ArrowLeftRightIcon, PlugZap2Icon } from 'lucide-react';
import { usePermissions } from '../../contexts/PermissionsProvider';

const Tabs = () => {
  const { pathname } = useLocation();
  const navigate = useNavigate();
  const { roles, permissions } = usePermissions(); // ← utilisation directe
  ...
  const links = [
    ...
    {
      name: 'Hors pont',
      isActive: pathname.includes('off-bridge'),
      link: '/incidents/off-bridge',
      icon: <TruckIcon className="h-4 w-4" />,
      requiredPermissions: [],
      requiredRoles: [],
    ...
    {
      name: 'Deplacement équipement',
      isActive: pathname.includes('movements'),
      link: '/incidents/movements',
      icon: <ArrowLeftRightIcon className="h-4 w-4" />,
      requiredPermissions: [],
      requiredRoles: [],
    ...
    {
      name: 'Maintenances',
      isActive: pathname.includes('maintenance') && !pathname.includes('type'),
      link: '/incidents/maintenance',
      icon: <WrenchScrewdriverIcon className="h-4 w-4" />,
      requiredPermissions: ['incident__view_maintenance'],
      requiredRoles: ['maintenance technician', 'HSE supervisor', 'IT technician', 'manager', 'coordinator'],
    ...
    {
      name: 'Equipements',
      isActive: pathname.includes('equipement'),
      link: '/incidents/equipement',
    ...
    {
      name: 'Reporting Cg',
      isActive: pathname.includes('reporting-cgs'),
    ...
    {
      name: 'Consommables',
      isActive: pathname.includes('consumables'),
    ...
    {
      name: 'Prix de pesée',
      isActive: pathname.includes('weighing-prices'),
    ...
    //   isActive: pathname.includes('reporting-supervisories'),
    ...
    // {
    //   name: 'Navires',
    //   isActive: pathname.includes('ships'),
    //   link: '/incidents/ships',
    //   icon: <LifebuoyIcon className="h-4 w-4" />,
    //   requiredPermissions: ['incident__view_ships'],
    //   requiredRoles: ['DEX', 'customer manager', 'manager', 'coordinator', 'RESPONSIBLE', 'CORDO', 'ROP'],
    ...
    {
      name: 'Type Equipements',
      isActive: pathname.includes('equipment-groups'),
      link: '/incidents/equipment-groups',
    ...
    {
      name: 'Domaines Equipements',
      isActive: pathname.includes('equipment-group-families'),
      link: '/incidents/equipment-group-families',
      icon: <ArchiveBoxIcon className="h-4 w-4" />,
      requiredPermissions: ['incident__view_equipment-families'],
      requiredRoles: ['IT technician', 'maintenance technician', 'HSE supervisor', 'manager', 'coordinator'],
    ...
    {
      name: "Causes d'incidents",
      isActive: pathname.includes('incident-cause'),
      link: '/incidents/incident-cause',
      icon: <Cog6ToothIcon className="h-4 w-4" />,
      requiredPermissions: ['incident__view_incident_causes'],
    ...
    {
      name: "Types d'incidents",
      isActive: pathname.includes('incident-type'),
      link: '/incidents/incident-type',
      icon: <Cog6ToothIcon className="h-4 w-4" />,
      requiredPermissions: ['incident__view_incident_types'],
      requiredRoles: ['IT technician', 'maintenance technician', 'manager', 'coordinator'],
  ...
  // Fonction qui détermine si un lien doit être affiché
  const shouldShowLink = (link) => {
    if (link.requiredPermissions.length === 0 && link.requiredRoles.length === 0) return true;
    const hasPermission = link.requiredPermissions.some((perm) => permissions.includes(perm));
    const hasRole = link.requiredRoles.some((role) => roles.includes(role));
  ...
  useEffect(() => {
    ...
    window.addEventListener('resize', checkScroll);
    return () => window.removeEventListener('resize', checkScroll);
  }, [pathname, roles, permissions]); // ← re‑calcule le défilement quand les données changent
            ...
            (link, index) =>
              shouldShowLink(link) && (
                <div
                  key={index}
                  ref={link.isActive ? activeTabRef : null}
                  className={`px-2 ${link.icon && 'flex items-center gap-2'} p-1 shadow-md ${
                    link.isActive ? 'bg-secondary text-white' : 'border-[1px] border-gray-300'
                  } rounded-full cursor-pointer text-sm font-semibold flex justify-center`}
                  onClick={() => navigate(link.link)}
                >
                  {link.icon}
                  <span>{link.name}</span>
