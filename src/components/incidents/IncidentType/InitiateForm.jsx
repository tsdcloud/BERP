...
const InitiateForm = ({onSucess, editData, onCancelEdit, userDomain = "PRIVILEGED"}) => {
  ...
  // Utiliser useMemo pour mémoriser les options de domaine
  ...
  }, [userDomain]); // Dépend uniquement de userDomain
  ...
  useEffect(() => {
    if (editData) {
      ...
      // En mode création, définir le domaine par défaut si l'utilisateur n'est pas privilégié
      if (userDomain !== "PRIVILEGED" && domainOptions.length === 1) {
        // Utiliser setTimeout pour éviter de modifier le formulaire pendant le rendu
        setTimeout(() => {
          setValue("domain", domainOptions[0].value || "");
        }, 0);
      }
    }
  }, [editData, setValue, reset, userDomain, onCancelEdit]); // Retirer domainOptions des dépendances

  // Effet séparé pour initialiser le domaine en mode création
  useEffect(() => {
    if (!editData && userDomain !== "PRIVILEGED" && domainOptions.length === 1) {
      setValue("domain", domainOptions[0].value || "");
    }
  }, [editData, userDomain, domainOptions, setValue]);
  ...
  // Déterminer si le sélecteur de domaine doit être désactivé
  const isDomainDisabled = useMemo(() => {
    return (userDomain !== "PRIVILEGED" && domainOptions.length === 1) || 
           (isEditMode && userDomain !== "PRIVILEGED" && editData?.domain !== userDomain);
  }, [userDomain, domainOptions.length, isEditMode, editData]);
