...
const InitiateForm = ({ onSuccess, editData, onCancelEdit }) => {
    ...
    const [isShiftLoading, setIsShiftLoading] = useState(true);
    const [isEmployeeLoading, setIsEmployeeLoading] = useState(false);
    const [isConsumableLoading, setIsConsumableLoading] = useState(false);
    const [isSubmitting, setIsSubmitting] = useState(false);

    // Consommables sélectionnés
    const [consumablesListSelected, setConsumablesListSelected] = useState([]);
    ...
    const onSubmit = async (data) => {
        ...
        if (data.numberPrepaidWeighDefinitivelyCompleted === "" || data.numberPrepaidWeighDefinitivelyCompleted === undefined) {
            toast.error("Le nombre de pesées prépayées définitivement réalisées est obligatoire");
            return;
        }
        ...
        try {
            ...
            const payload = {
                ...
                operators: operatorsList.map(op => op.value),
                hses: hsesList.map(h => h.value),
                consumables: consumablesListSelected.map(c => c.value),
                completeNumberWeighingsToBeBilled: parseInt(data.completeNumberWeighingsToBeBilled) || 0,
                completeNumberWeighingsBySpecies: parseInt(data.completeNumberWeighingsBySpecies) || 0,
                incompleteNumberWeighingsToBeBilled: parseInt(data.incompleteNumberWeighingsToBeBilled) || 0,
                incompleteNumberWeighingsBySpecies: parseInt(data.incompleteNumberWeighingsBySpecies) || 0,
                testNumberWeighingsBySpecies: parseInt(data.testNumberWeighingsBySpecies) || 0,
                // ✅ NOUVEAU : champ obligatoire, désormais collecté et envoyé
                numberPrepaidWeighDefinitivelyCompleted: parseInt(data.numberPrepaidWeighDefinitivelyCompleted) || 0,
                offBridgeNumber: parseInt(data.offBridgeNumber) || 0,
                numberIncidents: parseInt(data.numberIncidents) || 0,
                incidentDescription: data.incidentDescription || "",
                productionDescription: data.productionDescription || "",
                incomingCgId: data.incomingCgId,
                extractionFileUrl: extractionFileUrl,
                attachments: attachmentsUrls,
                firstWeighNumber: data.firstWeighNumber || null,
                lastWeighNumber: data.lastWeighNumber || null,
                firstWeighTractorNumber: data.firstWeighTractorNumber || null,
                lastWeighTractorNumber: data.lastWeighTractorNumber || null,
                firstWeighDate: data.firstWeighDate || null,
                lastWeighDate: data.lastWeighDate || null,
            };
    ...
    useEffect(() => {
        ...
        if (isEditMode && editData) {
            reset({
                ...
                incompleteNumberWeighingsBySpecies: editData.incompleteNumberWeighingsBySpecies,
                testNumberWeighingsBySpecies: editData.testNumberWeighingsBySpecies,
                numberPrepaidWeighDefinitivelyCompleted: editData.numberPrepaidWeighDefinitivelyCompleted,
                offBridgeNumber: editData.offBridgeNumber,
                numberIncidents: editData.numberIncidents,
                incidentDescription: editData.incidentDescription,
                productionDescription: editData.productionDescription,
            ...
            if (editData.outOfStockConsumableReportingCgs) {
                setConsumablesListSelected(editData.outOfStockConsumableReportingCgs.map(c => ({
                    name: c.consumable?.name || c.consumableId,
                    value: c.consumableId
                })));
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label className="text-sm font-medium">
                        Pesées prépayées effectuées définitivement <span className="text-red-500">*</span>
                    </label>
                    <input
                        type="number"
                        min="0"
                    ...
                    {errors.numberPrepaidWeighDefinitivelyCompleted && (
                        <p className="text-red-500 text-xs mt-1">{errors.numberPrepaidWeighDefinitivelyCompleted.message}</p>
                    )}
                </div>
            </div>
