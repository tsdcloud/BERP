...
// // import { CheckCircle, Upload, X, Trash2, Loader2 } from 'lucide-react';
...
// //     // États pour les fichiers (stockage local, pas d'upload immédiat)
// //     const [extractionFileFile, setExtractionFileFile] = useState(null); // File object
// //     const [attachmentFiles, setAttachmentFiles] = useState([]); // Array de File objects
...
// //     // ===== 4. UPLOAD DES FICHIERS (appelé pendant la soumission) =====
// //     // const uploadFileToServer = async (file) => {
// //     //     const formData = new FormData();
// //     //     formData.append('files', file); // Description: le backend attend 'files' (tableau)
// //     //     try {
...
// //         try {
// //             // 1. Upload du fichier d'extraction (si présent)
...
// //             } else if (isEditMode && editData?.extractionFileUrl) {
// //                 extractionFileUrl = editData.extractionFileUrl; // conserver l'ancien
// //             }
...
// //                     });
// //                 }
// //             } else if (isEditMode && editData?.attachments) {
// //                 attachmentsUrls.push(...editData.attachments);
// //             }
...
// //     // ===== 6. GESTION DES FICHIERS (sans upload immédiat) =====
...
// //     // ===== 7. GESTION DES LISTES MULTIPLES =====
...
// //     // ===== 8. CHARGEMENT INITIAL & MODE ÉDITION =====
// //     useEffect(() => {
// //         fetchSites();
// //         fetchShifts();
// //         fetchEmployees();
...
// //                 // On ne pré-remplit pas le fichier physique, mais on pourrait afficher un lien
// //                 // On laisse vide, l'utilisateur peut choisir un nouveau fichier
// //             }
// //             if (editData.attachments) {
// //                 // On ne pré-remplit pas les attachments physiques non plus
// //             }
...
// //                 </div>
// //                 <div>
// //                     <label className="text-sm font-medium">Passages sans pesée en espèce</label>
...
// //                     </div>
// //                 )}
// //                 {isEditMode && editData?.extractionFileUrl && !extractionFileFile && (
// //                     <p className="text-xs text-blue-500 mt-1">Fichier actuel : <a href={editData.extractionFileUrl} target="_blank" rel="noreferrer">voir</a></p>
// //                 )}
// //             </div>
...
// //                         <div key={idx} className="flex justify-between items-center bg-gray-100 p-1 rounded">
...
// //                         </div>
// //                     ))}
// //                     {isEditMode && editData?.attachments?.length > 0 && attachmentFiles.length === 0 && (
// //                         <p className="text-xs text-blue-500">Pièces jointes existantes : {editData.attachments.length} fichier(s)</p>
// //                     )}
// //                 </div>
// //             </div>
...
//     // États pour les fichiers (stockage local)
//     const [extractionFileFile, setExtractionFileFile] = useState(null);
//     const [attachmentFiles, setAttachmentFiles] = useState([]);
//     // Pour l'édition : conserver les anciennes pièces jointes (à ne pas réuploader)
...
//     // ===== 4. UPLOAD FICHIER (appelé pendant la soumission) =====
//     const uploadFileToServer = async (file) => {
//         const formData = new FormData();
//         formData.append('files', file);
//         const response = await fetch(`${URLS.INCIDENT_API}/files/upload`, {
...
//             // 2. Pièces jointes : nouvelles + existantes
//             const attachmentsUrls = [];
...
//                 }
//             }
//             // Anciennes pièces jointes (en édition)
...
//         if (file) {
//             setExtractionFileFile(file);
//             // En mode édition, si on choisit un nouveau fichier, on efface l'ancienne URL
...
//     // Supprimer une ancienne pièce jointe (en mode édition)
...
//                     initialValue={shifts.find(s => s.value === editData?.shiftId)}
//                 />
//                 {errors.shiftId && <p className="text-xs text-red-500 mt-1">{errors.shiftId.message}</p>}
//             </div>

//             {/* Opérateurs (obligatoire) */}
//             <div>
//                 <label className="block text-sm font-medium mb-1">Opérateurs <span className="text-red-500">*</span></label>
//                 <AutoComplete
//                     placeholder="Ajouter un opérateur"
...
//                     ))}
//                 </div>
//                 {operatorsList.length === 0 && <p className="text-xs text-red-500 mt-1">Au moins un opérateur requis</p>}
//             </div>
...
//                     ))}
//                 </div>
//                 {hsesList.length === 0 && <p className="text-xs text-red-500 mt-1">Au moins un HSE requis</p>}
//             </div>
...
//                         </div>
//                     ))}
//                     {/* Anciennes pièces jointes (mode édition) */}
