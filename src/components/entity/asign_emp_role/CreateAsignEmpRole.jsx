    ...
    useEffect(()=>{
      ...
      const handleSubmitDataFormAsignEmpRole = async(data) => {
        ...
        try {
            ...
            for (const rId of roleId) {
                try {
                    const response = await handlePost(
                        urlToCreateAsignEmpRole, 
                        {employeeId, roleId: rId, createdBy }, 
                        true
                    );
            ...
            if (successCount === roleId.length) {
                toast.success("Assignation créée avec succès", { duration: 2000 });
                setOpen(false);
                onSubmit();
                reset();
            } else if (!hasError && successCount > 0) {
                toast.warning(`${successCount} rôle(s) sur ${roleId.length} ont été assignés`, { duration: 3000 });
                setOpen(false);
                onSubmit();
                reset();
            }
            ...
            <form onSubmit={handleSubmit(handleSubmitDataFormAsignEmpRole)} className=''>
                 ...
                 <div className='mb-4'>
                    ...
                    <div className='my-3'>
                            ...
                            <div className='flex flex-wrap items-center overflow-y-auto h-[100px]'>
                                {showRole.map(item => (
                                    <div key={item?.id} 
                                    className={`flex font-mono items-center ml-1 px-2 py-2 border bg-secondary text-white rounded-sm
