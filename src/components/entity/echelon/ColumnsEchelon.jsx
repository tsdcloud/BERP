        ...
        try {
            const response = await handlePatch(urlToUpdate, data);
                if (response) {
                    ...
                    setDialogOpen(false);
                    toast.error("Erreur lors de la modification echelon", { duration: 5000 });
                }
        ...
        if (confirmation) {
            ...
            const urlToDisabledEchelon = `${URLS.ENTITY_API}/echelons/${id}`;
                    ...
                    try {
                        ...
                        console.error("Erreur lors de la désactivation echelon :", error);
                    }
            ...
            const urlToDisabledEchelon = `${URLS.ENTITY_API}/echelons/${id}`;
                    ...
                    try {
                            ...
                            console.log("response for deleted", response);
                                if (response) {
                                    setTimeout(()=>{
                                        toast.success("echelon enabled successfully", { duration: 5000});
                                        window.location.reload();
                                    },[200]);
                                }
                                else {
                                  toast.error("Erreur lors de la réactivation echelon", { duration: 5000 });
                                }
                            isDialogOpen && setDialogOpen(false);
                    }
                    catch(error){
                        console.error("Erreur lors de la réactivation echelon :", error);
                    }
                                ...
                                selectedEchelon && (
                                    <div className='flex flex-col text-black text-left space-y-3'>
                                        ...
                                        <div>
                                            ...
                                            <h3 className="font-bold text-sm">{selectedEchelon?.name}</h3>
                                        </div>
                                        <div>
                                            <p className="text-xs">Date de création</p>
                                            <h3 className="font-bold text-sm">{selectedEchelon?.createdAt.split("T")[0]}</h3>
                                        </div>
                                        <div>
                                            <p className="text-xs">Statut</p>
                                            <h3 className="font-bold text-sm">
                                                {selectedEchelon?.isActive ? "Actif" : "Désactivé"}
                                            </h3>
                                        </div>
                                    </div>
                                )
                    ...
                    <AlertDialogFooter>
                        ...
                        isEdited === false ? (
                            <div className='flex space-x-2'>
                                            {/* <div className='flex space-x-2'>
                                                { 
                                                    selectedEchelon?.isActive == false ? 
                                                        (
                                                                <AlertDialogAction
