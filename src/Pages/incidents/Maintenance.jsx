...
const Maintenance = () => {
    ...
    /**
     * Récupération des maintenances avec gestion des paramètres
     */
    const fetchMaintenances = useCallback(async (params = {}) => {
        ...
        try {
            ...
            // Gestion spéciale pour le statut (utilise 'status' directement)
            if (params.status) {
                queryParams.append('status', params.status);
            }
            // Gestion des autres filtres (type filter/value)
    ...
    const handleOnSelectChange = (evt) => {
        ...
        fetchMaintenances(); // réinitialise
    };

    /**
     * Changement de la valeur de recherche
     */
    const handleInputChange = async (evt) => {
        const value = evt.target.value;
        setInputValue(value);
        ...
        // Cas spécial pour le statut : on utilise 'status'
        if (selectValue === 'statut') {
            if (value) {
                const statusParams = { status: value };
                setCurrentFilter(statusParams);
                fetchMaintenances(statusParams);
            } else {
                setCurrentFilter(null);
                fetchMaintenances();
            }
        ...
        // Autres filtres
            ...
            const filterParams = { filter: selectValue, value: value };
            setCurrentFilter(filterParams);
            fetchMaintenances(filterParams);
        } else if (!value) {
            setCurrentFilter(null);
            fetchMaintenances();
        }
    ...
    const handlePageChange = (newPage) => {
        ...
        if (currentFilter) {
            Object.assign(params, currentFilter);
        }
        fetchMaintenances(params);
    };

    /**
     * Rendu du champ input en fonction du filtre sélectionné
     */
    const handleDisplayInput = (criteria) => {
        return (
    ...
    // Initialisation du filtre à partir du paramètre d'URL 'status'
    useEffect(() => {
        const statusParam = searchParams.get('status');
        if (statusParam && ['PENDING', 'CLOSED', 'UNDER_MAINTENANCE'].includes(statusParam)) {
            setSelectValue('statut');
            setInputValue(statusParam);
            const statusFilter = { status: statusParam };
            setCurrentFilter(statusFilter);
            fetchMaintenances(statusFilter);
        } else {
            fetchMaintenances();
        }
