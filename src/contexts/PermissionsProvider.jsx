...
const PermissionsProvider = ({ children }) => {
    ...
    useEffect(() => {
        const fetchPermissionsAndRoles = async () => {
            try {
                const employee = await getEmployee();
                if (!employee) return; // ✅ suppression de setIsLoading inexistant

                const employeeRoles       = await handleFetch(`${URLS.ENTITY_API}/employees/${employee?.id}/roles`);
                const employeePermissions = await handleFetch(`${URLS.ENTITY_API}/employees/${employee?.id}/permissions`);

                const empRoles = employeeRoles?.employeeRoles       ?? [];
                const empPerms = employeePermissions?.employeePermissions ?? [];

                // ✅ Extraction de roleName — cohérent avec le middleware et ReportingCg
                const formatedRoles = empRoles.map(er => er?.role?.roleName).filter(Boolean);
                const formatedPerms = empPerms.map(ep => ep?.permission?.permissionName).filter(Boolean);
